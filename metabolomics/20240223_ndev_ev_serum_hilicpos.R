...
filter.val <- read_tsv("results/blank_filtration/hilic_pos_exo_sec.txt")

# Call in hilic + data ----
hilic_pos <- read_tsv("HILIC_column/RAW_mzcalibrated_untargeted_featuretable_HILICpos.txt")
new_names <- gsub(".mzXML", "", names(hilic_pos))
names(hilic_pos) <- new_names
map <- read_tsv("HILIC_column/map_edited_hilic_pos.txt") %>% 
...
hilic_pos_compare <- hilic_pos_exo %>% 
    dplyr::select(mz_time, hilic_pos_map_compare$File.Name)

# Find features with differential abundance in the comparison groups
hilic_pos_compare_labels <- hilic_pos_compare %>% 
    t() %>% 
    as.data.frame() %>% 
    janitor::row_to_names(1) %>% 
    rownames_to_column(var = "File.Name") %>% 
    right_join(hilic_pos_map_compare, ., by = "File.Name")

# Features most abundant in NDEVs
hilic_pos_ndev <- hilic_pos_compare_labels %>% 
...
hilic_pos_ndev %>% write_tsv("results/NDEV/hilic_pos_serum_features_NDEV.txt")

# Features most abundant in total ev
hilic_pos_tev <- hilic_pos_compare_labels %>% 
...
hilic_pos_tev %>% write_tsv("results/NDEV/hilic_pos_serum_features_TotalEV.txt")

# Features most abundant in serum
hilic_pos_serum <- hilic_pos_compare_labels %>% 
...
hilic_pos_serum %>% write_tsv("results/NDEV/hilic_pos_serum_features_serum.txt")

hilic_pos_mean_all <- hilic_pos_compare_labels %>% 
...
hilic_pos_mean <- hilic_pos_compare_labels %>% 
    gather(key = "feature", value = "intensity", -type_compare, -File.Name) %>% 
    mutate_at("intensity", as.character) %>% 
    mutate_at("intensity", as.numeric) %>% 
    group_by(feature) %>%
...
zero_feature <- hilic_pos_mean %>% 
    filter(mean.int == 0)

# Select features most abundant in nEVs ----
all_top_100 <- hilic_pos_mean_all %>% 
    arrange(-mean.int) %>% 
    slice(1:100)
...
met_pca <- hilic_pos_compare_labels %>% 
...
png("results/NDEV/hilicpos_PCA.png", res = 300, units = "in", h = 3, w = 4)
...
hilic_pos_compare_labels %>% 
    dplyr::select(-File.Name, -type_compare, -all_of(zero_feature$feature)) %>% 
    map_df(~as.character(.x)) %>% 
    map_df(~as.numeric(.x)) %>% 
    map_df(~replacezero(.x)) %>% 
    mutate(File.Name = hilic_pos_compare_labels$File.Name) %>% 
    mutate(type_compare = hilic_pos_compare_labels$type_compare) %>% 
    select(File.Name, type_compare, everything()) %>% 
    write_tsv("nev_metabolomics/hilicpos_nev_serum_tev_replaced.txt")
