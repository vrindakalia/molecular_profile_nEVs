...
filter.val <- read_tsv("results/blank_filtration/C18_pos_exo_sec.txt")

# Call in C18 + data ----
C18_pos <- read_tsv("C18_column/RAW_mzcalibrated_untargeted_featuretable_C18pos.txt")
new_names <- gsub(".mzXML", "", names(C18_pos))
names(C18_pos) <- new_names
map <- read_tsv("C18_column/map_edited_C18_pos.txt") %>% 
...
C18_pos_compare <- C18_pos_exo %>% 
    dplyr::select(mz_time, C18_pos_map_compare$File.Name)

# Find features with differential abundance in the comparison groups
C18_pos_compare_labels <- C18_pos_compare %>% 
    t() %>% 
    as.data.frame() %>% 
    janitor::row_to_names(1) %>% 
    rownames_to_column(var = "File.Name") %>% 
    right_join(C18_pos_map_compare, ., by = "File.Name")

# Features most abundant in NDEVs
C18_pos_ndev <- C18_pos_compare_labels %>% 
...
C18_pos_ndev %>% write_tsv("results/NDEV/C18_pos_serum_features_NDEV.txt")

# Features most abundant in total ev
C18_pos_tev <- C18_pos_compare_labels %>% 
...
C18_pos_tev %>% write_tsv("results/NDEV/C18_pos_serum_features_TotalEV.txt")

# Features most abundant in serum
C18_pos_serum <- C18_pos_compare_labels %>% 
...
C18_pos_serum %>% write_tsv("results/NDEV/C18_pos_serum_features_serum.txt")

C18_pos_mean_all <- C18_pos_compare_labels %>% 
...
C18_pos_mean <- C18_pos_compare_labels %>% 
    gather(key = "feature", value = "intensity", -type_compare, -File.Name) %>% 
    mutate_at("intensity", as.character) %>% 
    mutate_at("intensity", as.numeric) %>% 
    group_by(feature) %>%
...
zero_feature <- C18_pos_mean %>% 
    filter(mean.int == 0)

# Select features most abundant in nEVs ----
all_top_100 <- C18_pos_mean_all %>% 
    arrange(-mean.int) %>% 
    slice(1:100)
...
met_pca <- C18_pos_compare_labels %>% 
...
png("results/NDEV/C18pos_PCA.png", res = 300, units = "in", h = 3, w = 4)
factoextra::fviz_pca_ind(met_pca, axes = c(1,2), habillage = hilic_pos_compare_labels$type_compare, 
                         geom = c("point"), title = "C18 +") 
dev.off()


C18_pos_compare_labels %>% 
    dplyr::select(-File.Name, -type_compare, -all_of(zero_feature$feature)) %>% 
    map_df(~as.character(.x)) %>% 
    map_df(~as.numeric(.x)) %>% 
    map_df(~replacezero(.x)) %>% 
    mutate(File.Name = C18_pos_compare_labels$File.Name) %>% 
    mutate(type_compare = C18_pos_compare_labels$type_compare) %>% 
    select(File.Name, type_compare, everything()) %>% 
    write_tsv("nev_metabolomics/C18pos_nev_serum_tev_replaced.txt")
