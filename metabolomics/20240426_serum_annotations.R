...
serum_C18neg <- read_tsv("results/NDEV/C18_neg_serum_features_serum.txt")
...
data_lib %>% write_tsv("results/NDEV/level1-5_c18_neg_features_serum.txt")
...
# Call in the feature list of interest
serum_C18pos <- read_tsv("results/NDEV/C18_pos_serum_features_serum.txt")
...
data_lib %>% write_tsv("results/NDEV/level1-5_c18_pos_features_serum.txt")
...
# Call in the feature list of interest
serum_hilicpos <- read_tsv("results/NDEV/hilic_pos_serum_features_serum.txt")

dataA <- serum_hilicpos %>% 
    separate(feature, into = c("mz", "time"), sep = "_", remove = FALSE) %>% 
    dplyr::select(mz, time, everything()) %>% 
    mutate_at("mz", as.numeric) %>% 
    mutate_at("time", as.numeric) %>% 
...
data_lib %>% write_tsv("results/NDEV/level1-5_hilic_pos_features_serum.txt")
...
# Call in the feature list of interest
serum_hilicneg <- read_tsv("results/NDEV/hilic_neg_serum_features_serum.txt")

dataA <- serum_hilicneg %>% 
    separate(feature, into = c("mz", "time"), sep = "_", remove = FALSE) %>% 
    dplyr::select(mz, time, everything()) %>% 
    mutate_at("mz", as.numeric) %>% 
    mutate_at("time", as.numeric) %>% 
...
data_lib %>% write_tsv("results/NDEV/level1-5_hilic_neg_features_serum.txt")
