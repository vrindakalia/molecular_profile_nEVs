...
# Super class ----
ndev_super <- read_csv("results/NDEV/enrichment/20240426_ndev_super_class/msea_ora_result.csv") %>% 
    mutate(matrix = "NDEV") %>% 
    dplyr::rename(Name = 1)
TotalEV_super <- read_csv("results/NDEV/enrichment/20240426_Totalev_super_class/msea_ora_result.csv") %>% 
    mutate(matrix = "TotalEV") %>% 
    dplyr::rename(Name = 1)
serum_super <- read_csv("results/NDEV/enrichment/20240426_serum_super_class/msea_ora_result.csv") %>% 
    mutate(matrix = "Serum") %>% 
    dplyr::rename(Name = 1)
...
names_other <- super_class_all %>% 
    ...
    distinct(Name) %>% 
    mutate(color = tol21rainbow[1:10]) %>% 
...
# Main class ----
ndev_main <- read_csv("results/NDEV/enrichment/20240426_ndev_main_class/msea_ora_result.csv") %>% 
    mutate(matrix = "NDEV") %>% 
    dplyr::rename(Name = 1)
Totalev_main <- read_csv("results/NDEV/enrichment/20240426_Totalev_main_class/msea_ora_result.csv") %>% 
    mutate(matrix = "TotalEV") %>% 
    dplyr::rename(Name = 1)
serum_main <- read_csv("results/NDEV/enrichment/20240426_serum_main_class/msea_ora_result.csv") %>% 
    mutate(matrix = "Serum") %>% 
    dplyr::rename(Name = 1)
...
# Sub class ----
ndev_sub <- read_csv("results/NDEV/enrichment/20240426_ndev_sub_class/msea_ora_result.csv") %>% 
    mutate(matrix = "NDEV") %>% 
    dplyr::rename(Name = 1)
TotalEV_sub <- read_csv("results/NDEV/enrichment/20240426_Totalev_sub_class/msea_ora_result.csv") %>% 
    mutate(matrix = "TotalEV") %>% 
    dplyr::rename(Name = 1)
serum_sub <- read_csv("results/NDEV/enrichment/20240426_serum_sub_class/msea_ora_result.csv") %>% 
    mutate(matrix = "Serum") %>% 
    dplyr::rename(Name = 1)
...
names_sub <- sub_class_all %>% 
    ...
    mutate(class = case_when(prop > 2.5 ~ Name,
                             prop <= 2.5 ~ "z_Other")) %>% 
    distinct(class) %>% 
    mutate(color = tol21rainbow[c(1:6)]) %>% 
    column_to_rownames(var = "class")
