...
C18neg <- read_tsv("C18_column/RAW_mzcalibrated_untargeted_featuretable_C18neg.txt") %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# C18 neg
lib_neg <- read.xlsx("/Users/vk2316/Documents/Baccarelli_lab/EVs/with_DW/HRE_Lab/library/230103-C18neg_Merged_InchKey_Filtered_Confirmed_ChemicalDB.xlsx") %>% 
...
std_neg_ann_filter_mz <- neg_overlap %>% 
    ...
    unite("mz_time", c(mz, time), sep = "_", remove = FALSE) %>%
    merge(., C18neg_meanint, by = "mz_time") %>% 
    group_by(mz_time) %>% 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# C18 neg
ann_neg <- read_csv("xMSannotator/C18 neg/Stage5.csv")
...
neg_ann_level135 %>% write_tsv("results/C18neg_annotations_level1-5.txt")
