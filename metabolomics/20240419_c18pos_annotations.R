...
C18pos <- read_tsv("C18_column/RAW_mzcalibrated_untargeted_featuretable_C18pos.txt") %>%
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# C18 pos
lib_pos <- read.xlsx("/Users/vk2316/Documents/Baccarelli_lab/EVs/with_DW/HRE_Lab/library/230103-C18pos_Merged_InchKey_Filtered_Confirmed_ChemicalDB.xlsx") %>% 
...
std_pos_ann_filter_mz <- pos_overlap %>% 
    ...
    unite("mz_time", c(mz, time), sep = "_", remove = FALSE) %>%
    merge(., C18pos_meanint, by = "mz_time") %>% 
    group_by(mz_time) %>% 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
# C18 POS
ann_pos <- read_csv("xMSannotator/C18 POS/Stage5.csv")
...
pos_ann_level135 %>% write_tsv("results/C18pos_annotations_level1-5.txt")
